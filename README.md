...
- **Пропорции:** ширина, высота и длительность берутся из готового файла (ffprobe), а не
  из метаданных сайта; к видео прикладывается превью тех же пропорций. Видео с флагом
  поворота (вертикальные с телефона) или неквадратными пикселями (анаморфное, SAR ≠ 1)
  перекодируются в H.264 с квадратными пикселями — многие клиенты Telegram эти флаги
  игнорируют и показывают такие видео сплющенными.
